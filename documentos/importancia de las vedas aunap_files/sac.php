...
function filter_smilies(s) {
	for (var i = 0; i < smilies.length; i++) {
		...
		var imgsrc = 'https://www.aunap.gov.co/wp-includes/images/smilies/' + smilies[i][1];
...
var GetChaturl  = 'https://www.aunap.gov.co/wp-content/plugins/simple-ajax-chat/simple-ajax-chat-core.php?sacGetChat=yes';
var SendChaturl = 'https://www.aunap.gov.co/wp-content/plugins/simple-ajax-chat/simple-ajax-chat-core.php?sacSendChat=yes';
...
function receiveChatText() {
	...
	if (httpReceiveChat.readyState == 4 || httpReceiveChat.readyState == 0) {
		...
		var nonce = '&sac_nonce_receive=5f5cf9f367';
...
function insertNewContent(liName,liText,lastResponse, liUrl, liId) {
	...
	if ('Notification' in window) {
		sacCustomNotify('Nuevo mensaje de chat', liName + ': ' + sac_apply_filters(liText) + "\r\n" + '(' + datetime + ')', window.location.href);
	}
	...
	insertO.insertBefore(oLi, insertO.firstChild);
		...
		jQuery.post('https://www.aunap.gov.co/wp-content/plugins/simple-ajax-chat/includes/sac-check-user.php', 'sac_user=' + encodeURIComponent(liName), function(response){
...
function checkName() {
	...
	if (chat_name != sacCookie) {
		document.cookie = 'sacUserName=' + chat_name + '; expires=Sun, 16 Apr 2023 01:13:14 UTC; samesite=strict;';
	}
...
function checkUrl() {
	...
	if (currentName.value != sacCookie) {
		document.cookie = 'sacUrl=' + currentName.value + '; expires=Sun, 16 Apr 2023 01:13:14 UTC; samesite=strict;';
		return;
	}
...
jQuery(document).ready(function($){
	...
	// nonce
	var url = 'https://www.aunap.gov.co/wp-content/plugins/simple-ajax-chat/resources/nonce.php';
...
function sacCustomNotify(title, desc, url) {
	if (window.Notification && Notification.permission == 'granted') {
		var notification = new Notification(title, {
			icon: 'https://www.aunap.gov.co/wp-content/plugins/simple-ajax-chat/resources/sac-400.png',
			body: desc,
		});
		if (url) {
